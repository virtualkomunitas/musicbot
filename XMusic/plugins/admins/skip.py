...
async def skip(cli, message: Message, _, chat_id):
    if not len(message.command) < 2:
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await XMusic.stop_stream(chat_id)
                except:
                    return
        except:
            try:
    ...
    if "live_" in queued:
        ...
        try:
            await XMusic.skip_stream(chat_id, link, video=status)
        except Exception:
        ...
        try:
            await XMusic.skip_stream(chat_id, file_path, video=status)
        except Exception:
            return await mystic.edit_text(_["call_9"])
        ...
        try:
            await XMusic.skip_stream(chat_id, queued, video=status)
        except Exception:
            return await message.reply_text(_["call_9"])
        if videoid == "telegram":
            button = telegram_markup(_)
